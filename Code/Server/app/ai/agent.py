...
try:
    from app.ai.extensions.agent_accel import compute_sequences as cython_compute_sequences
except ImportError:
    cython_compute_sequences = None

try:
    from app.ai.extensions.search_accel import search_best_move as cython_search_best_move
except ImportError:
...
class Agent:
    ...
    def get_heuristic(self, game: Caro) -> int:
        ...
        score = self.compute(game._get_all_rows()) + \
            self.compute(game._get_all_diagonals()) + \
            self.compute(game._get_all_cols())
        with self.cache_lock:
    ...
    def _has_forced_vcf_win(self, game: Caro, attacker_piece: str, ply_left: int) -> bool:
        ...
        if game.XO == attacker_piece:
            ...
            for move in attack_moves:
                ...
                new_game._make_move(move[0], move[1])
        ...
        for response in responses:
            new_game = copy.deepcopy(game)
            new_game._make_move(response[0], response[1])
    ...
    def _find_immediate_win_move(self, game: Caro, piece: str) -> list[int] | None:
        ...
        for x, y in winning_candidates:
            ...
            trial.XO = piece
            trial._make_move(x, y)
            winner = trial._get_winner()
    ...
    def get_move(self, game: Caro) -> list[list[int]]:
        ...
        if len(game.last_move) < 1:
            possible_moves = game._get_possible_moves()
            if not possible_moves:
                return None
...
if __name__ == '__main__':
    ...
    print(best_move)
    game._make_move(best_move[0], best_move[1])
